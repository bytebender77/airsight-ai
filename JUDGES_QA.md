...
### 7. Why use Regression (exact values) instead of Classification (categories)?
A common question might be why we predict the exact PM2.5 value (`35.1`) instead of just a category like "Moderate".

*   **Granularity:** PM2.5 health impacts are linear. Predicting `24 µg/m³` vs `26 µg/m³` matters for long-term health monitoring, even if both fall into the same "category".
*   **One-Way Flexibility:** You can always convert a number into a category (e.g., `>50` = Hazardous). You **cannot** convert a category like "Poor" back into an exact measurement for scientific modeling.
*   **Trend Sensitivity:** Regression allows our "Trend Detection" logic to work. Classification hides small but significant increases in pollution that our model can catch early.
*   **AQI Compatibility:** Our dashboard automatically maps these regression outputs to the color-coded **WHO Risk Scale**, giving the judges the best of both worlds: scientific precision + easy-to-read categories.
